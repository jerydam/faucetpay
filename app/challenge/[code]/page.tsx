...
// ── Floating Chat ─────────────────────────────────────────────────────────────
// FIX: Positioned above bottom-safe-area. During game the nav is hidden so
//      bottom-6 is fine; in lobby the nav is also hidden (BottomNav returns null
//      on game pages), so bottom-6 / right-4 is safe everywhere.
...
function FloatingChat({ messages, myWallet, chatInput, setChatInput, onSend, chatBottomRef, unreadCount }: FloatingChatProps) {
  ...
  return (
    // bottom-6 right-4 — nav is hidden on game pages so no overlap
...
export default function ChallengePage() {
      ...
      .then(d => {
        ...
        const playerEntries = Object.entries(d.challenge.players ?? {}).map(
          ([wallet, data]: [string, any]) => ({
            ...
            // FIX: map avatar_url correctly from API response
  ...
  const handleRefreshLobby = useCallback(() => {
    ...
    fetch(`${API_BASE_URL}/api/challenge/${code}`)
      ...
      .then(d => {
        if (!d.success) return;
        const entries = Object.entries(d.challenge.players ?? {}).map(
          ([wallet, data]: [string, any]) => ({
            walletAddress: wallet,
            username:      data.username,
            points:        data.points,
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/challenge/${code}/join`, {
      ...
      .then(d => {
        ...
        if (d.success) toast.info(`Stake agreed at ${agreedStake} ${challenge.token} — approve the transaction to lock it in!`);
      })
    ...
    ws.onmessage = async (ev) => {
      ...
      switch (msg.type) {
        case "state_sync": {
          ...
          setChallenge(c);
          // FIX: Always use server state for txVerified — don't merge with stale local state.
          // ready is reset server-side on reconnect; txVerified persists since stake is on-chain.
          setPlayers(
            Object.entries(c.players ?? {}).map(([w, d]: [string, any]) => ({
              walletAddress: w,
              username:      d.username,
              points:        d.points,
              ready:         d.ready,
        ...
        case "player_joined": {
          ...
          setPlayers(prev => {
            ...
            return [...prev, {
              ...
              ready:         false,
              txVerified:    false,
              // FIX: read avatar from both possible field names
        ...
        case "stake_verified": {
          setPlayers(prev => {
            ...
            if (!exists) return [...prev, {
              walletAddress: wallet, username, points: 0, ready: false, txVerified: true,
        ...
        case "round_announce": {
          ...
          const cdInterval = setInterval(() => {
            setCountdownVal(prev => {
              if (prev <= 1) { clearInterval(cdInterval); return prev; }
              return prev - 1;
            });
          }, 1000);
          break;
        ...
        case "rematch_invite_accepted": {
          if (msg.acceptorWallet?.toLowerCase() !== myWallet) {
            ...
            await handleRematchCreate({
              code, userWalletAddress: userWalletAddress!, challenge, router,
  ...
  }, [code, userWalletAddress, myWallet, startTimer, username, clearRematchTimers, challenge, router]);
  ...
  if (phase === "game_over") {
    ...
    return (
      <>
        {globalOverlays}
        <div className="fixed inset-0 bg-background flex flex-col overflow-auto z-[60]">
              ...
              {canRematch && (
                <button
                  onClick={() => sendRematchInvite({
                    ...
                    setRematchPending, setRematchCountdown, rematchTimerRef, rematchTimeoutRef,
                  })}
  ...
  if (phase === "countdown") {
    return (
      <>
        {globalOverlays}
        {/* z-[60] ensures it covers the bottom nav (z-50) */}
  ...
  if ((phase === "question" || phase === "reveal") && currentQ) {
    ...
    return (
      // FIX: z-[60] to sit above bottom nav (z-50). Nav is also hidden on game
      // pages via BottomNav's isGamePage check, but belt-and-suspenders here.
      <div className="fixed inset-0 bg-background flex flex-col overflow-hidden z-[60]">
        ...
        {/* Question + mini leaderboard (reveal) */}
                    ...
                    <div key={p.walletAddress} className="flex items-center justify-between px-4 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <Avatar className="h-6 w-6">
                          {/* FIX: explicitly pass undefined when empty so fallback renders */}
                          <AvatarImage src={p.avatarUrl || undefined} />
                          <AvatarFallback className="text-[8px]">{p.username.slice(0,2).toUpperCase()}</AvatarFallback>
                        </Avatar>
        ...
        {/* Options grid
            FIX: Always grid-cols-2 with explicit padding so buttons have consistent
            sizing in both question and reveal states. */}
        <div className="w-full max-w-2xl mx-auto px-4 grid grid-cols-2 gap-3 pb-8 shrink-0">
          {currentQ.options.map(opt => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-base opacity-90">{style.shape}</span>
  ...
  if (phase === "round_end") {
    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col items-center justify-center z-[60] px-4">
  ...
  // ── Pre-join (direct entry, not from pre-lobby) ───────────────────────────────
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-md border-b border-border">
          <div className="max-w-4xl mx-auto px-4 h-16 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {/* FIX: Refresh button in lobby header */}
              <button
                ...
                {players.map(p => {
                  ...
                  return (
                    <div key={p.walletAddress} className={cn(
                      ...
                      p.ready      ? "border-emerald-400/40 bg-emerald-500/5"
                        : p.txVerified ? "border-blue-400/30 bg-blue-500/5"
                        : "border-border bg-muted/20",
                    )}>
                      {/* FIX: AvatarImage src is undefined when empty so fallback renders */}
