"use client";
/**
 * /app/challenge/page.tsx  — Quiz Hub
 * Refactored to match backend: /api/challenge/* + /api/players/*
 * Backend lobby endpoint: GET /api/challenge/lobby
 * No /api/challenge/list or DELETE endpoint exists — uses lobby + history instead.
 */
...
export default function QuizListPage() {
 async function resolveDestination(
  ...
  if (!userWallet) {
    return `/challenge/${code}/pre-lobby`;
  }
              ...
              <div className="divide-y divide-slate-100 dark:divide-white/[0.05]">
                {historyChallenges.map(c => {
                  ...
                  return (
                    <button
                      key={(c as any).code}
                      onClick={() => router.push(`/challenge/${(c as any).code}`)}
